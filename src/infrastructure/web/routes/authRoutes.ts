...
export function createAuthRoutes(
  ...
  router.post(
    "/create-moderator",
    //authMiddleware, // Requiere token válido
    //roleMiddleware(['moderator']), // Solo administradores pueden crear moderadores
    rateLimitMiddleware(5, 60), // 5 creaciones por hora
    async (req, res) => {
      try {
        await authController.createModerator(req, res);
      } catch (error) {
        console.error("❌ Error en ruta /create-moderator:", error);
        res.status(500).json({
          success: false,
          error: "Internal server error",
          details: error instanceof Error ? error.message : String(error),
  ...
  router.post(
    ...
    //authMiddleware, // Requiere token válido
    //roleMiddleware(['administrator']), // Solo administradores pueden crear otros administradores
    rateLimitMiddleware(3, 60), // 3 creaciones por hora
    async (req, res) => {
      try {
        await authController.createAdministrator(req, res);
      } catch (error) {
        console.error("❌ Error en ruta /create-administrator:", error);
        res.status(500).json({
          success: false,
          error: "Internal server error",
          details: error instanceof Error ? error.message : String(error),
  ...
  router.post(
    "/parental-consent/approve/:token",
  ...
  // Incluir las rutas de administración CRUD
  router.use("/", createAdminRoutes(adminUserController));
